...
- **Iterative humanization loop** — runs `/humanizer` repeatedly, scored by `codex` and `gemini` CLIs each pass; loops until avg AI-likelihood < 10 (max 5 passes).
...
4. **Iterative humanization** — Loop until avg AI-likelihood < 10 (multi-model detect)
...
Audience → Research + Site Profile → Outline + Expert Review → [GATE 1]
        ...
        → Link Curation (3-way grade, caps) → Iterative Humanize Loop (max 5, avg<10)
...
Runs in a scored loop: `/humanizer` → 3-perspective filter (loss/gap/hallucination) → `codex` + `gemini` rate AI-likelihood → repeat until avg < 10 or max 5 iterations. Cap reached → surfaces full score trace to user.

### External CLI Reviewers
- `codex exec "<prompt>"` — independent OpenAI-model verifier
- `gemini -p "<prompt>"` — independent Google-model verifier
